...
export async function generateImage({
    ...
    if (currentMode === 'gemini') {
        requestUrl = 'api/gemini_proxy.php?endpoint=edit';
    ...
    try {
        ...
        if (canStream) {
            const streamUrl = 'api/openai_proxy.php?endpoint=generations_stream';
...
export function downloadBatch(batchId, prompt) {
    const downloadUrl = `api/zip_download.php?batchId=${batchId}`;
    const link = document.createElement('a');
