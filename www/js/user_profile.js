...
async function openMyImagesModal() {
    ...
    const res = await fetch('api/list_images.php', { headers });
    if (!res.ok) return;
    ...
    for (const [key, items] of byBatch.entries()) {
        ...
        batchInfo.className = 'inline-flex items-center gap-1 px-2 py-1 rounded-full text-xs font-medium bg-indigo-100 text-indigo-700 dark:bg-indigo-900/30 dark:text-indigo-300 shadow-sm';
        batchInfo.innerHTML = `
            <svg class="w-3 h-3" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
        ...
        quality.className = `inline-flex items-center gap-1 px-2 py-1 rounded-full text-xs font-medium ${qualityColor} shadow-sm`;
        quality.innerHTML = `
        ...
        ar.className = 'inline-flex items-center gap-1 px-2 py-1 rounded-full text-xs font-medium bg-pink-100 text-pink-700 dark:bg-pink-900/30 dark:text-pink-300 shadow-sm';
        ar.innerHTML = `
            <svg class="w-3 h-3" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
    ...
    if (delBtn) delBtn.onclick = async () => {
        ...
        for (const s of sel) {
            if (s.batchId) {
                await fetch('api/delete_image.php', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ batchId: s.batchId }) });
            } else if (s.filename) {
                await fetch('api/delete_image.php', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ filename: s.filename }) });
            }
        }
        await openMyImagesModal();
    };
        ...
        for (const s of sel) {
            if (s.batchId) {
                ...
                for (const f of files) {
                    ...
                    await fetch('api/set_private.php', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ filename: name, private: 1 }) });
                }
            } else if (s.filename) {
                await fetch('api/set_private.php', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ filename: s.filename, private: 1 }) });
            }
        }
        await openMyImagesModal();
    };
    ...
    if (pubBtn) pubBtn.onclick = async () => {
        ...
        for (const s of sel) {
            if (s.batchId) {
                ...
                for (const f of files) {
                    const name = f.file.split('/').pop();
                    await fetch('api/set_private.php', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ filename: name, private: 0 }) });
                }
            } else if (s.filename) {
                await fetch('api/set_private.php', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ filename: s.filename, private: 0 }) });
            }
        }
        await openMyImagesModal();
    };
    ...
    if (zipBtn) zipBtn.onclick = async () => {
        ...
        try {
            const resp = await fetch('api/zip_download.php', {
                method: 'POST',
...
function initMyImagesTabSwitching() {
    const tabButtons = document.querySelectorAll('.myimages-tab-btn');
    const tabContents = document.querySelectorAll('.myimages-tab-content');
    ...
    tabButtons.forEach(button => {
        button.addEventListener('click', () => {
            ...
            tabButtons.forEach(btn => {
                btn.classList.remove('active', 'bg-gradient-to-r', 'from-indigo-500', 'to-purple-600', 'text-white', 'shadow-md');
                btn.classList.add('text-gray-700', 'dark:text-gray-300', 'hover:bg-white', 'dark:hover:bg-slate-800', 'hover:shadow-sm');
            });
            
            // Add active state to clicked button
            button.classList.add('active', 'bg-gradient-to-r', 'from-indigo-500', 'to-purple-600', 'text-white', 'shadow-md');
            button.classList.remove('text-gray-700', 'dark:text-gray-300', 'hover:bg-white', 'dark:hover:bg-slate-800', 'hover:shadow-sm');
            ...
            const targetContent = document.getElementById(`myimages-tab-${targetTab}`);
            if (targetContent) {
