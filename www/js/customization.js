...
function initLanguageSelect() {
    ...
    languageSelect.addEventListener('change', async () => {
        ...
        try {
            const response = await fetch('api/update_customization.php', {
                method: 'POST',
        ...
        if (btn) {
            btn.addEventListener('click', () => {
                ...
                if (isAdmin) {
                    // Load OpenAI API Key
                    if (openaiKeyInput) {
                        fetch('api/api_keys.php?provider=openai')
                            .then(res => res.json())
                            .then(data => {
                    ...
                    // Load Gemini API Key
                    if (geminiKeyInput) {
                        fetch('api/api_keys.php?provider=gemini')
                            .then(res => res.json())
                            .then(data => {
...
async function loadCustomization(featuresContainer) {
    try {
        const response = await fetch('./api/get_customization.php');
...
function updatePageContent(data) {
    ...
    if (featuresDisplay && Array.isArray(data.features)) {
        ...
        data.features.forEach((feature) => {
            ...
            featureElement.className = 'flex items-center gap-2 bg-white/80 dark:bg-slate-800/80 backdrop-blur-sm px-4 py-2 rounded-xl border border-gray-100 dark:border-slate-700 shadow-md hover:shadow-lg transition-shadow duration-300';
