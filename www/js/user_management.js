...
export function initUserManagement() {
    ...
    fetch('api/session_auth.php?action=status')
        .then(res => res.json())
        .then(data => {
            if (data.role === 'admin') {
    ...
    if (addUserForm) {
        addUserForm.addEventListener('submit', async (e) => {
            ...
            try {
                const response = await fetch('api/user_management.php?action=add', {
                    method: 'POST',
                    headers: {
...
function updateUserList() {
    ...
    Object.entries(currentUsers).forEach(([username, data]) => {
        ...
        userRow.className = 'flex items-center justify-between p-4 border-b border-gray-100 dark:border-slate-700 hover:bg-gray-50 dark:hover:bg-slate-800/50 transition-colors duration-200';
        ...
        renameBtn.className = 'bg-green-50 dark:bg-green-500/10 hover:bg-green-100 dark:hover:bg-green-500/20 text-green-600 dark:text-green-400 px-3 py-1.5 rounded-lg text-sm font-medium transition shadow-sm hover:shadow-md';
        renameBtn.textContent = translate('userActions.rename');
        renameBtn.addEventListener('click', () => promptUserRename(username));

        // Rolle ändern
        const roleSelect = document.createElement('select');
        roleSelect.className = 'bg-white dark:bg-slate-800 border border-gray-200 dark:border-slate-700 rounded-lg px-3 py-1.5 text-sm text-gray-700 dark:text-gray-300 shadow-sm';
        ...
        changePasswordBtn.className = 'bg-indigo-50 dark:bg-indigo-500/10 hover:bg-indigo-100 dark:hover:bg-indigo-500/20 text-indigo-600 dark:text-indigo-400 px-3 py-1.5 rounded-lg text-sm font-medium transition shadow-sm hover:shadow-md';
        ...
        deleteBtn.className = 'bg-red-50 dark:bg-red-500/10 hover:bg-red-100 dark:hover:bg-red-500/20 text-red-600 dark:text-red-400 px-3 py-1.5 rounded-lg text-sm font-medium transition shadow-sm hover:shadow-md';
...
async function updateUserRole(username, newRole) {
    try {
        const response = await fetch('api/user_management.php?action=update', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json'
            },
...
async function deleteUser(username) {
    ...
    try {
        const response = await fetch('api/user_management.php?action=delete', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json'
            },
...
async function promptUserRename(username) {
    ...
    try {
        const response = await fetch('api/user_management.php?action=rename', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json'
            },
